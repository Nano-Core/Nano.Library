...
namespace Nano.Services.Globale.Models.Collections
{
    /// <summary>
    /// Internal class containing <see cref="IDictionary{TKey,TValue}"/> property <see cref="PhoneNumberPrefix.List"/> 
    /// of <see cref="Country.IsoAlpha2"/> as keys and the phone number prefix as value.
    /// </summary>
    internal static class PhoneNumberPrefix
    {
        /// <summary>
        /// A <see cref="IDictionary{TKey,TValue}"/> of <see cref="Country.IsoAlpha2"/> as keys and prefix as value
        /// </summary>
        internal static IDictionary<string, string> List => new Dictionary<string, string>
        {
            {"AT", "+43"},
            {"BE", "+32"},
            {"BG", "+359"},
        ...
        public static KeyValuePair<string, string> Find(string e164PhoneNumber)
        {
            if (e164PhoneNumber == null)
                throw new ArgumentNullException(nameof(e164PhoneNumber));

            return e164PhoneNumber.StartsWith("+1") 
                ? PhoneNumberPrefix.phoneNumberPrefixesNorthAmerica.FirstOrDefault(x => e164PhoneNumber.StartsWith(x.Value)) 
                : PhoneNumberPrefix.List.FirstOrDefault(x => e164PhoneNumber.StartsWith(x.Value));
        }
    }
}