...
namespace Nano.Config.Extensions
{
    ...
    public static class ServiceCollectionExtensions
    {
        ...
        {
            // TODO: LOGGING: MOre flexible config, so provider like ES can work.
            ...
            where TProvider : class, IEventingProvider
        {
            services
                ...
                .AddSingleton(provider =>
                {
                    ...
                    // TODO: EVENTING: ConnectionConfiguration:  return RabbitHutch.CreateBus(configuration, register => { });
            ...
            where TContext : DbContext, IDbContext
        {
            ...
            services
                ...
                .AddDbContext<TContext>((provider, builder) =>
                {
                    ...
                    if (type == typeof(SqlServerDataProvider))
                    {
                        builder.UseSqlServer(connectionString, x => x.MaxBatchSize(batchSize));
                        return;
                    }

                    throw new NotSupportedException($"Unsupported Provider Type: {type.FullName}");
                });
        ...
        /// <returns>The <see cref="IServiceCollection"/>.</returns>
        internal static IServiceCollection AddAppContext(this IServiceCollection services, IConfiguration configuration)
        {
            if (services == null)
                throw new ArgumentNullException(nameof(services));
